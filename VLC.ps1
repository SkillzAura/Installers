...
$getHTML = Invoke-Webrequest -Uri $vlcURL -UseBasicParsing
$name = $getHTML.Links.href | Where-Object{$_ -like "vlc-*.exe"} | select -First 1
$vlcURL = "https://download.videolan.org/vlc/last/win64/$name"

$outFile = "$ENV:Temp\vlc64.exe"

(New-Object System.Net.WebClient).DownloadFile($vlcURL, $outFile)

Start-Process -FilePath $outFile -Args "/L=1033 /S" -Wait; Remove-Item $outFile

New-Item -ItemType "File" -Path "$env:appdata\vlc\vlcrc" -Force
$MultilineComment = @"
###
###  vlc 3.0.20
###
...
# Encoder (string)
#sout-avi-encoder=VLC Media Player - 3.0.20 Vetinari
...
# SRT maximum payload size (bytes) (integer)
#payload-size=1316

# SRT maximum bandwidth ceiling (bytes) (integer)
#bandwidth-overhead-limit=25

# Crypto key length in bytes (integer)
#key-length=16

#  SRT Stream ID (string)
#streamid=
...
"@
Set-Content -Path "$env:appdata\vlc\vlcrc" -Value $MultilineComment -Force

Remove-Item "C:\Users\Public\Desktop\VLC Media Player.lnk" -Force
Remove-Item "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\VideoLAN" -Recurse -Force
